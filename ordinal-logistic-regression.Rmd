...
# Add a column indicating if the dog was found to have compulsion.
df.mod$compulsion <- ifelse(df.mod$q02_main_3==1, 1, 0)

# Convert "acquisition_source" into "rescue" column.
df.mod$rescue <- ifelse(
  (df.mod$acquisition_source==1 | df.mod$acquisition_source==4), 1, 0)
...
# Fit a logistic regression.
f <- as.formula(paste0(outcome, "~", ". + alt_med_type_4*pain"))
print(f)
coef <- rep(0, length(df.agg)-1)
zeta <- seq(from=0, to=1, length.out=7)
start <- c(coef, zeta)
m <- polr(f, data=df.agg, Hess=TRUE, start=start)
summary(m)
