...
df.agg$agg_avg <-ifelse(df.agg$agg_avg <= 4, 0, 1)
summary(df.agg)
...
#summary(m)
df_out <- as.data.frame(tidy(m, conf.int=FALSE, exponentiate=FALSE))
df_out$sig <- ''
...
odds.ratio <- as.vector(exp(coef(m)))
iters <- 500
b.ors <- matrix(, nrow=length(odds.ratio), ncol=iters)
for (i in 1:iters) {
  df.tmp <- df.agg[sample(nrow(df.agg), nrow(df.agg), replace=T), ]
  m <- glm(f, data=df.tmp, family="binomial")
  b.ors[,i] <- exp(coef(m))
}
ci <- t(apply(b.ors, 1, quantile, probs=c(0.025, 0.975), na.rm=TRUE))
ci.low <- round(ci[,1], 2)
ci.high <- round(ci[,2], 2)

df_out <- cbind(df_out, odds.ratio, ci.low, ci.high)
for (i in 1:nrow(df_out)) {
  if (is.na(df_out[i, 'ci.low']) | is.na(df_out[i, 'ci.high'])) next
  if ((df_out[i, 'ci.low'] < 1) & (df_out[i, 'ci.high'] > 1)) {
    df_out[i, 'sig'] <- ''
  }
}
...
df.agg <- apply_binary_response_criteria(
    get_agg_data_frame(df.mod, predictors, outcome, excludes))
# Collapse the Likert scale to be dichotomous.
df.agg$agg_conf <-ifelse(df.agg$agg_conf <= 4, 0, 1)
df.agg$agg_conf <- as.factor(df.agg$agg_conf)
summary(df.agg)
...
df.agg <- apply_binary_response_criteria(
  get_agg_data_frame(df.mod, predictors, outcome, excludes))
# Collapse the Likert scale to be dichotomous.
df.agg$agg_idh <-ifelse(df.agg$agg_idh <= 4, 0, 1)
summary(df.agg)

f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df.agg, family="binomial")

df_out <- as.data.frame(tidy(m, conf.int=TRUE))
df_out$p.value <- round(p.adjust(df_out$p.value, 'BH'), 3)
df_out$sig <- ''
df_out[df_out$p.value <= .05, 'sig'] <- '*'
df_out[df_out$p.value <= .01, 'sig'] <- '**'
